...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            if (parts.Length != 4)
            {
                Console.WriteLine("Command not valid, Create requires a name and type of a gradebook, if it's weighted (true / false).");
                return;
            }

            var name = parts[1];
            var typeOfGB = parts[2].ToLower();
            ...
            if (typeOfGB == "standard")
            {
                gradeBook = new StandardGradeBook(name, isWeighted);
            }
            else if (typeOfGB == "ranked")
            {
                gradeBook = new RankedGradeBook(name, isWeighted);
            }
            else
            {
                Console.WriteLine(typeOfGB + " is not a supported type of gradebook, please try again");
                return;
            }
